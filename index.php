        ...
        <div style="width:95%;" class="container">
            ...
            <div class = 'row'>
                <div class = 'col'>
                    ...
                    <hr/>
                    <?php
                    $prep_stmt = $conn->prepare("SELECT *, DATE_FORMAT(posted, '%m/%d/%y') as create_date_formatted FROM updates WHERE posted BETWEEN NOW() - INTERVAL 30 DAY AND NOW() AND category='Game Update' DESC");
                    $prep_stmt->execute();
                    $row = $prep_stmt->fetchAll();
                    $count = $prep_stmt->rowCount();
                    for($x = 0; $x < $count; $x++) {
